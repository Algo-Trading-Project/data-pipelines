...
class GetQuoteDataOperator(BaseOperator):
    ...
    def __get_next_start_date(self, coinapi_pair):
        ...
        most_recent_data_date = coinapi_pair['latest_scrape_date_quote']
    ...
    def __upload_new_quote_data(self, new_quote_data):
        if len(new_quote_data) == 0:
            return
        
        data_to_uplaod = json.dumps(new_quote_data).replace('[', '').replace(']', '').replace('},', '}')
        key = 'eth_data/quote_data/coinapi_pair_quote_data_1_hour.json.{}'.format(self.s3_file_chunk_num)
    ...
    def __update_coinapi_pairs_metadata(self, time_start, coinapi_pairs_df, coinapi_pair):
        ...
        coinapi_pairs_df.loc[predicate, 'latest_scrape_date_quote'] = new_latest_scrape_date

        return coinapi_pairs_df
    
    def __delete_quote_data_from_s3(self):
        keys_to_delete = self.s3_connection.list_keys(
            bucket_name = 'project-poseidon-data',
            prefix = 'eth_data/quote_data'
        )
    ...
    def __get_latest_quote_data(self, coinapi_symbol_id, time_start):
        ...
        api_request_url = 'https://rest.coinapi.io/v1/quotes/{}/history?time_start={}&limit={}'.format(coinapi_symbol_id, time_start, 1)
    ...
    def execute(self, context):
        # Delete quote data potentially stored in S3 from previous task runs
        self.__delete_quote_data_from_s3()
        ...
        for i in range(len(coinapi_pairs_df)):
            ...
            while True:
                ...
                latest_quote_data_for_pair = self.__get_latest_quote_data(coinapi_symbol_id = coinapi_symbol_id, time_start = time_start)
                
                # If request didn't succeed
                if type(latest_quote_data_for_pair) == int:
                    ...
                    if latest_quote_data_for_pair == 429:
                        self.__upload_new_quote_data(quote_data)
                ...
                # If request succeeded
                    ...
                    if len(latest_quote_data_for_pair) == 0:
                    ...
                    # If request returned a non-empty response
                        ...
                        # Add to list of quote data for current token
                        quote_data.extend(latest_quote_data_for_pair)
