...
with DAG(
    ...
    move_staging_data_to_prod = RedshiftSQLOperator(
        ...
        queries = [
            ...
            """
            INSERT INTO administrator.eth_data.gas_used (
                SELECT *
                FROM 
                    administrator.eth_data.stg_gas_used
                WHERE 
                    block_no >= (
                        SELECT MAX(block_no)
                        FROM administrator.eth_data.gas_used
                    )
            );
            """,

            """
            DELETE FROM administrator.eth_data.stg_gas_used
            WHERE 
                block_no <= (
                    SELECT MAX(block_no)
                    FROM administrator.eth_data.gas_used
