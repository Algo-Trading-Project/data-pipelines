...
with DAG(
    ...
    move_staging_data_to_prod = RedshiftSQLOperator(
        ...
        queries = [
            ...
            """
            INSERT INTO administrator.eth_data.transfer (
                SELECT *
                FROM 
                    administrator.eth_data.stg_transfer
                WHERE 
                    block_no >= (
                        SELECT MAX(block_no)
                        FROM administrator.eth_data.transfer
                    )
            );
            """,

            """
            DELETE FROM administrator.eth_data.stg_transfer
            WHERE 
                block_no <= (
                    SELECT MAX(block_no)
                    FROM administrator.eth_data.transfer
