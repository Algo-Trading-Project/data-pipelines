...
def agg_trade_data_1d(date):
    ...
    # Input path: read all tokens' data for that date
    path = f"{RAW_SPOT_TRADES.uri}/*/time_period_open={date}/*.parquet"
    
    # Output path
    output_path = f"{AGG_SPOT_TRADES.uri}/"
...
with DAG(
    ...
    finish = DummyOperator(task_id="finish", outlets=[AGG_SPOT_TRADES])