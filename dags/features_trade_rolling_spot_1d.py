...
from datasets import (
    AGG_SPOT_TRADES, ROLLING_SPOT_TRADES
)
...
with DAG(
    ...
    schedule=[AGG_SPOT_TRADES],
    catchup=False,
) as dag:
    compute = PythonOperator(
        task_id="calc_spot_trade_rollups",
        python_callable=_run_duckdb_rolling,
        op_kwargs={
            'exec_date': "{{ ds }}",
            'input_dir': AGG_SPOT_TRADES.uri,
            'output_dir': ROLLING_SPOT_TRADES.uri,
        },
    )
    finish = EmptyOperator(task_id="finish", outlets=[ROLLING_SPOT_TRADES])
    compute >> finish
