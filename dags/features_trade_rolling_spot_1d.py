...
with DAG(
    ...
    AGG_SPOT_TRADES = 's3://base-44-data/data/trade_data/agg'
    ROLLING_SPOT_TRADES = 's3://base-44-data/data/trade_data/rolling'

    wait_for_agg = ExternalTaskSensor(
        task_id="wait_for_agg_spot_trade_data_1d",
        external_dag_id="agg_spot_trade_data_1d",
    ...
    compute = PythonOperator(
        task_id="compute_spot_trade_rolling_features",
        python_callable=_run_duckdb_rolling,
        op_kwargs={
            'exec_date': "{{ ds }}",
            'input_dir': AGG_SPOT_TRADES,
            'output_dir': ROLLING_SPOT_TRADES,
        },
    )
